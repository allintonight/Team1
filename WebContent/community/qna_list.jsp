...
<thead class="thead-dark">
			...
			%>
				...
				<td scope="col">
					...
					<%if(secret==1){
						%><td><a href="qna_secret.jsp?no=<%=no%>&no=<%=no%>&pageNUM=<%=pageNUM%>">
...
</svg><%= title %><%if(comment != null)out.print("[답변완료]"); %>
						</a>
					</td><%
					}else{ %>
					<td scope="col"><a href="qna_show.jsp?no=<%=no%>&no=<%=no%>&pageNUM=<%=pageNUM%>">
						<%= title %><%if(comment != null)out.print("[답변완료]"); %>
						</a>
					</td scope="col"><%} %>
					<td scope="col">
						<%= name %>
